...
def create_pdf_report(df_f, month_name, budget, total_spent, user_name):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(95, 8, f"Monthly Budget: INR {budget:,.2f}")
    pdf.cell(95, 8, f"Total Spent: INR {total_spent:,.2f}", ln=True)
    
    remaining = budget - total_spent
    status = "ON TRACK" if remaining >= 0 else "OVER BUDGET"
    pdf.cell(190, 8, f"Status: {status} (Difference: INR {abs(remaining):,.2f})", ln=True)
    pdf.ln(5)
    
    # Category Breakdown
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 10, "2. Category Breakdown", ln=True)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(60, 8, "Category", border=1)
    pdf.cell(60, 8, "Amount (INR)", border=1)
    pdf.cell(70, 8, "% of Monthly Spend", border=1, ln=True)
    
    pdf.set_font("Arial", '', 10)
    by_cat = df_f.groupby("category")["amount"].sum().sort_values(ascending=False)
    for cat, amt in by_cat.items():
        pct = (amt / total_spent * 100) if total_spent > 0 else 0
        pdf.cell(60, 8, str(cat), border=1)
        pdf.cell(60, 8, f"{amt:,.2f}", border=1)
        pdf.cell(70, 8, f"{pct:.1f}%", border=1, ln=True)
    pdf.ln(5)
    
    # Recent Transactions
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(190, 10, "3. Recent Transactions (Top 15)", ln=True)
    pdf.set_font("Arial", 'B', 10)
    ...
    top_tx = df_f.sort_values("amount", ascending=False).head(15)
    for _, row in top_tx.iterrows():
        pdf.cell(30, 8, str(row['date'].strftime('%Y-%m-%d')), border=1)
        pdf.cell(80, 8, str(row['description'])[:40], border=1)
        pdf.cell(40, 8, str(row['category']), border=1)
        pdf.cell(40, 8, f"{row['amount']:,.2f}", border=1, ln=True)
        
    # Footer
    pdf.ln(10)
    pdf.set_font("Arial", 'I', 8)
    pdf.cell(190, 10, f"Generated by SpendSmart AI Assistant on {datetime.now().strftime('%Y-%m-%d %H:%M')}", align='C')
...
with tab2:
    if df_all.empty:
        ...
        # Export
        st.markdown('<div class="sh" style="margin-top:1.5rem">Download Report</div>', unsafe_allow_html=True)
        d1, d2 = st.columns(2)
        month_label = datetime.strptime(sel, "%Y-%m").strftime("%B %Y")
        try:
            pdf_bytes = create_pdf_report(df_f, month_label, budget, total, first_name)
            with d1:
                st.download_button(
                    label="📥 PDF Report",
                    data=pdf_bytes,
                    file_name=f"SpendSmart_Report_{sel}.pdf",
                    mime="application/pdf",
                    use_container_width=True
                )
        except Exception as e:
            with d1: st.error(f"PDF Error: {e}")
